...
public class StringUtil {
	public static String appendUrlParams(String url, String param) {
		if (url.contains("?")) {
			return url + "&" + param;
		}
		return url + "?" + param;
	...
	public static String getAppendUrl(String url) {
		if (url.contains("?")) {
	...
	public static String format(Context context, int msgId, Object... strings) {

		MessageFormat msg = new MessageFormat(context.getString(msgId));
		return msg.format(strings);
	}
	...
	public static long stringToLong(String src) {
		return stringToLong(src, 0);
	}

	public static long stringToLong(String src, long def) {
		long ret = def;
		try {
			if (!isNull(src)) {
				ret = Long.parseLong(src);
			} else {
				Loggers.d("stringToLong: src is empty");
			}
		} catch (Throwable e) {
			e.printStackTrace();
			ret = def;
	...
	public static int stringToInt(String src) {
		return stringToInt(src, 0);
	}

	public static int stringToInt(String src, int def) {
		int ret = def;
		try {
			if (!isNull(src)) {
				ret = Integer.parseInt(src);
			} else {
				Loggers.d("stringToInt: src is empty");
			}
		} catch (Throwable e) {
			e.printStackTrace();
			ret = def;
	...
	public static boolean stringToBoolean(String src) {
		return stringToBoolean(src, false);
	}

	public static boolean stringToBoolean(String src, boolean def) {
		boolean ret = def;
		try {
			if (stringToInt(src) > 0) {
				ret = true;
			}
		} catch (Throwable e) {
			e.printStackTrace();
